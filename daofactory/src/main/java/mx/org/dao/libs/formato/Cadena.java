...
public final class Cadena {
  ...
  private static final char[] CARACTER= {'}','{','*','»','$','%','&','+','~','\\',(char) 9 ,(char) 13,(char) 10,(char) 8 ,(char) 11,(char) 12,(char) 27,'Ä','Å','Á','Â','Ë','È','Ê','Í','Ï','Ì','Î','Ö','Ó','Ò','Î','À','Ü','Ú','Ù','Û','É','â','ä','à','å','á','é','ê','ë','è','ï','î','ì','í','ô','ö','ò','ó','ü','û','ù','ú','"'};
	private static final char[] REMPLAZO= {']','[',(char) 32,(char) 39,(char) 32,(char) 32,(char) 32,(char) 32,(char) 32,(char) 32,(char) 32,(char) 32,(char) 32,(char) 32,(char) 32,(char) 32,(char) 32,'A','A','A','A','E','E','E','I','I','I','I','O','O','O','I','A','U','U','U','U','E','a','a','a','a','a','e','e','e','e','i','i','i','i','o','o','o','o','u','u','u','u',(char) 32};
  ...
  /**
   * Elimina prefijos de una cadena dada, recibiendolo como parametro devolviendo la cadena sin prefijo
   *
   * @param cadena String a evaluar
   * @param prefijo String a buscar
   * @return
   */
  public static String eliminarPrefijo(String cadena, String prefijo) {
    String regresar=cadena;
    if (cadena.contains(prefijo))
      regresar = cadena.substring(cadena.lastIndexOf(prefijo) + prefijo.length(), cadena.length());
    return regresar;
  }//
  /**
   * Elimina postfijos de una cadena dada, recibiendolo como parametro parte del postfijo devolviendo la cadena sin postfijo
   *
   * @param cadena String a evaluar
   * @param postfijo String a buscar
   * @return
   */
  public static String eliminarPostfijo(String cadena, String postfijo) {
    String regresar=cadena;
    if (cadena.contains(postfijo))
      regresar = cadena.substring(0, cadena.lastIndexOf(postfijo));
    return regresar;
  }//
  ...
  public static int eliminar(String cadena, String prefijo) {
	...
	/**
	 * Formatea el nombre de una persona con May�sculas y min�sculas
	 *
	 * @return
   ...
   *
   * @param nombre String del nombre de una persona
   * @param longitudMinima Integer minimo del tamaño de una palabra
   * @return
   */
  public static String nombrePersona(String nombre, Integer longitudMinima) {
    StringBuilder sb = new StringBuilder();
    StringTokenizer st = new StringTokenizer(nombre, " ", false);
    String token = null;
    while (st.hasMoreTokens()) {
      token = st.nextToken();
      if (token.length() >= longitudMinima) {
  ...
  /**
   * Formatea el cadena con la primera letra may�sculas y las que le seguien en min�sculas
   *
  ...
  /**
   * Regresa una cadena con solo las letras, los numeros y el gui�n bajo (_) de una cadena y omite los dem�s caracteres
   *
  ...
  /**
   * Regresa un verdadero si la cadena es un null o viene vac�a
   *
   * @param cadena String a evaluar
   * @return
   */
  ...
  /**
   * Regresa un '' si la cadena es nulla o viene vac�a y regresa el valor si la cadena no es vac�a
   *
   * @param cadena String a evaluar
   * @return
   */
   ...
   *
   * @param value objeto que se le aplicar� el .toString(); no tienen que ser tipos de datos primitivos
   * @param text parametro que devolver� si el objeto value es null
   * @return
   */
  ...
  public static String toSqlName(String name) {
    ...
    for (char c : name.toCharArray()) {
      if(regresar.length()> 0 && Character.isUpperCase(c))
        regresar.append('_');
      regresar.append(Character.toLowerCase(c));
    } // for
    return regresar.toString();
  }
  ...
  /**
   * Revisa si una cadena que contenga un sentencia sql hace uso de order by, de ser as� devuelve la misma cadena pero
   ...
   *
   * @param params Mapa de parametros a evaluar
   * @param token caracter que separar� el key del valor
   * @return
   */
  ...
  /**
   * Formatea el nombre de un archivo con el estandar de Iktan Ej. toFileName("resutadosPreliminares") =
   * xIktan_Domingo,16 de Enero de 2015 08:24:05
   *
   * @param name
   * @return
   */
  ...
  /**
   * Recorta una descripcion a un determinado tama�o y le concatena a la descripcion ... al final
   *
  ...
  public static String tranformaAcentoPorHtmlAcute(String cadena) {
    String regresar = cadena;
    regresar = regresar.replaceAll("�", "&aacute;");
    regresar = regresar.replaceAll("�", "&eacute;");
    regresar = regresar.replaceAll("�", "&iacute;");
    regresar = regresar.replaceAll("�", "&oacute;");
    regresar = regresar.replaceAll("�", "&uacute;");
    regresar = regresar.replaceAll("�", "&Aacute;");
    regresar = regresar.replaceAll("�", "&Eacute;");
    regresar = regresar.replaceAll("�", "&Iacute;");
    regresar = regresar.replaceAll("�", "&Oacute;");
    regresar = regresar.replaceAll("�", "&Uacute;");
    regresar = regresar.replaceAll("�", "&ntilde;");
    regresar = regresar.replaceAll("�", "&Ntilde;");
    return regresar;
}
...
"          nvl(herramientas.obten_valor_de(tr_diagnosticos_tmp.descripcion, 'foliohog'), 0), tc_consistencias.consecutivo,to_number(folioviv), tc_imagenes.imagen";
  ...
  public static String replaceParams(String sql, Map params) {
	...
	public static final String rellenaConCerosI(String cadena, Integer longitud){
		String regresar = null;
		try{
			regresar= rellenar(cadena, longitud, '0', true);
		} // try
		catch(Exception e){
			Error.mensaje(e);
		} // catch
	...
	public static final String rellenaConCeros(int numeroEntrante, Integer longitud){
		String regresar = null;
		String cadena = String.valueOf(numeroEntrante);
		try{
			for(int x = cadena.length(); x < longitud; x++){
				cadena = '0'+cadena;
			} // for
			regresar = cadena;
		} // try
	...
	public static final String limpiaCadena (String cadena){
		String regresar= null;
		try{
		  for (int i= 0; i< CARACTER.length; i++){
        cadena= cadena.replace(CARACTER[i], REMPLAZO[i]);
			} // for
      regresar= cadena;
		} // try
		catch (Exception e){
	...
	public static final String valid(Object value){
  	String regresar= "";
		try{
  	  if (value!= null)
  ...
  public static String extractFileExtension (String fileName) {
    String regresar= null;
    try{
      regresar= fileName.substring(0, fileName.contains(Constantes.SEPARADOR_PROPIEDADES)? fileName.lastIndexOf(Constantes.SEPARADOR_PROPIEDADES): fileName.length());
    } // try
    catch (Exception e){
      Error.mensaje(e);
    } // catch
